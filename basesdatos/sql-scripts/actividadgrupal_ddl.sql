...
CREATE TABLE IF NOT EXISTS `actividadgrupal_new`.`anotacion` (
  ...
  `tipo_anotacion_identificador` INT NOT NULL,
  PRIMARY KEY (`identificador`, `tipo_anotacion_identificador`),
  INDEX `fk_anotacion_gen1_idx` (`gen_identificador` ASC) VISIBLE,
  INDEX `fk_anotacion_tipo_anotacion1_idx` (`tipo_anotacion_identificador` ASC) VISIBLE,
  CONSTRAINT `fk_anotacion_gen`
    FOREIGN KEY (`gen_identificador`)
    REFERENCES `actividadgrupal_new`.`gen` (`identificador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_anotacion_tipo_anotacion1`
    FOREIGN KEY (`tipo_anotacion_identificador`)
    REFERENCES `actividadgrupal_new`.`tipo_anotacion` (`identificador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
